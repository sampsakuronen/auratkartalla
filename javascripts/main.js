...
(function() {
  var PLOW_MAX_SPEED, activeMarkers, activePolylines, addMapLine, clearMap, createPlowTrail, createPlowsOnMap, displayNotification, distance, dropMapMarker, getActivePlows, getPlowJobColor, initializeGoogleMaps, map, msToH, pair, populateMap, snowAPI, toRad, unpair;
  ...
  distance = function(lat1, lng1, lat2, lng2) {
    var R, lat1rad, lat2rad, lng1rad, lng2rad, x, y;
    R = 6371;
    lat1rad = toRad(lat1);
    lat2rad = toRad(lat2);
    lng1rad = toRad(lng1);
  ...
  pair = function(arr) {
    if (arr.length < 2) {
      ...
      return _.range(2, arr.length + 1).map(function(i) {
        return arr.slice(i - 2, i);
      });
    }
  };
  ...
  unpair = function(arr) {
    if (arr.length < 1) {
      ...
      return arr.map(_.first).concat([_.last(_.last(arr))]);
    }
  };
  ...
  addMapLine = function(plowData, plowJobId) {
    var filteredPlowData, plowTrailColor, polyline, polylinePath;
    plowTrailColor = getPlowJobColor(plowJobId);
    filteredPlowData = unpair(pair(plowData).map(function(pair) {
      var d, t, x1, x2;
      x1 = pair[0], x2 = pair[1];
      ...
      return x.speed < PLOW_MAX_SPEED;
    }).map(function(x) {
      return x.pair;
    }));
    polylinePath = _.reduce(filteredPlowData, (function(accu, x) {
